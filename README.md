...
Create a process-based `ProcNexus` runner or, with `threaded=True`, a thread-based `ThreadNexus` runner from a callable.
...
* `threaded`: when `False`, use process workers; when `True`, use thread workers and return `ThreadNexus`.
...
### `ProcNexus.start() -> None` / `ThreadNexus.start() -> None`
...
### `ProcNexus.join(timeout=None) -> None` / `ThreadNexus.join(timeout=None) -> None`
...
## 🕒 History
### Unreleased
* Added `threaded=True` to `nexus()` for creating `ThreadNexus` runners backed by `multiprocessing.pool.ThreadPool`.
