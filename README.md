...
## 🧩 API
### `nexus(func, processes=None) -> ProcNexus`
### `nexus(func, threads=None) -> ThreadNexus`
...
### `start() -> None`
Start executing all queued tasks. With `processes=0`, `processes=None`, `threads=0`, or `threads=None`, this computes immediately in the current process; otherwise it starts the selected worker pool asynchronously.
